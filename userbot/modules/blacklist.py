# Copyright (C) 2021 KenHV

from sqlalchemy.exc import IntegrityError

from userbot import CMD_HELP
from userbot.events import register


@register(outgoing=True, pattern=r"^\.blacklist (.*)")
async def blacklist(event):
    """Adds given chat to blacklist."""
    try:
        from userbot.modules.sql_helper.blacklist_sql import add_blacklist
    except IntegrityError:
        return await event.edit("`Berjalan di mode NON-SQL!`")

    try:
        chat_id = int(event.pattern_match.group(1))
    except ValueError:
        chat_id = event.pattern_match.group(1)

    try:
        chat_id = await event.client.get_peer_id(chat_id)
    except Exception:
        return await event.edit("**Kesalahan**: Nama pengguna/ID yang diberikan tidak valid.")

    try:
        add_blacklist(str(chat_id))
    except IntegrityError:
        return await event.edit("`Obrolan yang diberikan sudah masuk daftar hitam.`")

    await event.edit("`Obrolan yang diberikan masuk daftar hitam.`")


@register(outgoing=True, pattern=r"^\.unblacklist (.*)")
async def unblacklist(event):
    """Unblacklists given chat."""
    try:
        from userbot.modules.sql_helper.blacklist_sql import (
            del_blacklist,
            get_blacklist,
        )
    except IntegrityError:
        return await event.edit("`Berjalan di mode NON-SQL!`")

    chat_id = event.pattern_match.group(1)
    try:
        chat_id = str(await event.client.get_peer_id(chat_id))
    except Exception:
        pass  # this way, deleted chats can be unblacklisted

    if chat_id == "all":
        from userbot.modules.sql_helper.blacklist_sql import del_blacklist_all

        del_blacklist_all()
        return await event.edit("`Menghapus semua daftar hitam!`")

    id_exists = False
    for i in get_blacklist():
        if chat_id == i.chat_id:
            id_exists = True

    if not id_exists:
        return await event.edit("`Obrolan ini tidak masuk daftar hitam.`")

    del_blacklist(chat_id)
    await event.edit("`Obrolan yang diberikan tidak masuk daftar hitam!`")


@register(outgoing=True, pattern=r"^\.blacklists$")
async def list_blacklist(event):
    """Lists all blacklisted chats."""
    try:
        from userbot.modules.sql_helper.blacklist_sql import get_blacklist
    except IntegrityError:
        return await event.edit("`Berjalan di mode NON-SQL!`")

    chat_list = get_blacklist()
    if not chat_list:
        return await event.edit("`Anda belum memasukkan obrolan apa pun ke daftar hitam!`")

    msg = "**Obrolan daftar hitam :**\n\n"

    for i in chat_list:
        try:
            chat = await event.client.get_entity(int(i.chat_id))
            chat = f"{chat.title} | `{i.chat_id}`"
        except (TypeError, ValueError):
            chat = f"__Tidak dapat mengambil info obrolan__ | `{i.chat_id}`"

        msg += f"• {chat}\n"

    await event.edit(msg)


CMD_HELP.update(
    {
        "blacklist": "**Menonaktifkan semua fungsi userbot pada grup yang masuk daftar hitam.**"
        "\n\n`.blacklist [nama pengguna/id]`"
        "\n➥  Memasukkan obrolan ke dalam daftar hitam."
        "\n\n`.unblacklist [nama pengguna/id]`"
        "\n➥  Menghapus obrolan dari daftar hitam."
        "\n\n`.unblacklist all`"
        "\n➥  Menghapus semua obrolan dari daftar hitam."
        "\n\n`.blacklists`"
        "\n➥  Menampilkan daftar semua obrolan yang masuk daftar hitam."
    }
)
