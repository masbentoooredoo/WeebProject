...
if __ is not None:
    if "uc?id=" in G_DRIVE_FOLDER_ID:
        LOGS.info("G_DRIVE_FOLDER_ID bukan folderURL yang valid...")
...
async def upload(gdrive, service, file_path, file_name, mimeType):
    ...
    body = {
        "name": file_name,
        "description": "Diunggah dari Telegram menggunakan WeebProject userbot.",
        "mimeType": mimeType,
    }
    try:
        if parent_Id is not None:
...
CMD_HELP.update(
    {
        ...
        "\n\n`.gdreset`"
        "\n➥  Setel ulang token Anda jika terjadi sesuatu yang buruk atau ubah akun drive."
        "\n\n`.gd`"
        "\n➥  Unggah file dari lokal atau uri/url/drivelink ke google drive. "
        "Untuk drivelink, unggah hanya jika Anda menginginkannya."
        "\n\n`.gdabort`"
        ...
        "\n➥  Salin file atau folder ke dalam Drive Anda."
        "\n\n**CATATAN** :"
