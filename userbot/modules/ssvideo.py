...
async def ssvideo(framecap):
    if not framecap.reply_to_msg_id:
        return await framecap.edit("`Balas video!`")
    reply_message = await framecap.get_reply_message()
    if not reply_message.media:
        return await framecap.edit("`Balas sebuah video!`")
    try:
        frame = int(framecap.pattern_match.group(1))
        if frame > 10:
            return await framecap.edit("`Hei...jangan terlalu banyak`")
    except BaseException:
        return await framecap.edit("`Harap masukkan jumlah bingkai!`")
    if (
        reply_message.photo
        or (
        ...
        return await framecap.edit("`File tidak didukung!`")
    c_time = time.time()
    ...
    try:
        await framecap.edit("`Sedang memproses...`")
...
CMD_HELP.update(
    {
        "ssvideo": "`.ssvideo [kisi]`"
        "\n➥  Tangkap bingkai video dengan [kisi] x [kisi]."
        "\n__*maksimal kisi adalah 10.__"
    }
)
